...
`docs/openapi.yaml` (OpenAPI 3.1) is the **source of truth** for the read-only API under
`/api/scheduling/v1`, not a generated by-product. Three guards keep it honest:

- **Route drift, both directions** — an endpoint without a spec entry and a spec entry without
  an endpoint both fail the build (`OpenApiContractTests`). Verified with a negative control.
- **Shape drift** — every documented schema is compared property-by-property against its DTO,
  and every declared schema must be bound to one, so a schema cannot quietly stop being checked.
- **Generated-client gate (CI)** — a TypeScript client is generated from the spec on every
  build; a generation failure is a spec bug and fails here rather than in the consumer's
  pipeline. The generated file is checked for the operations it must contain, because a
  generator can emit an empty-but-valid file.

Every endpoint sits behind `RequireEnabledModule("spaceos.scheduling")`, applied to the route
GROUP so a route added later cannot forget it. Tenant filtering appears nowhere in the endpoint
code: the EF query filter and the RLS policies do it.
